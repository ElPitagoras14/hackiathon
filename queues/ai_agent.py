...
SCORING_WEIGHTS = {
    "average_ccash_flow": 0.20,
    "debt_ratio": 0.10,
    "income_variability": 0.10,
    "platform_reviews": 0.10,
    "social_media_activity": 0.05,
    "suppliers_reviews": 0.10,
    "customer_reviews": 0.05,
    "payment_compliance": 0.15,
}
...
def _load_social_json(path: Path):
    ...
    engagement = (
        ((avg_likes + avg_comments) / followers_f) if followers_f else 0.0
    )
    posts_last_7 = (
        sum(1 for d in last_days if 0 <= d <= 6) if last_days else None
    )
...
def analyze_company(uid: str):
    base = Path("generated")
    estado_pdf = base / f"{uid}-balance.pdf"
    flujo_pdf = base / f"{uid}-flujo.pdf"
    integral_pdf = base / f"{uid}-integral.pdf"
    social_json_path = base / f"{uid}.json"

    sales = (
        extract_sales_from_pdf(str(integral_pdf))
        if integral_pdf.exists()
        else None
    )
    cash_flow = (
        extract_operating_cashflow_from_pdf(str(flujo_pdf))
        if flujo_pdf.exists()
        else None
    )
    ...
    current_solvency = (
        _safe_div(assets, liabilities) if assets and liabilities else None
    )
    asset_turnover = _safe_div(sales, assets) if assets else None
    cashflow_to_debt = (
        _safe_div(cash_flow, liabilities) if liabilities else None
    )
    ...
    vals = {
        ...
        "current_solvency": (
            current_solvency if current_solvency is not None else 0.0
        ),
        "asset_turnover": (
            asset_turnover if asset_turnover is not None else 0.0
        ),
    ...
    breakdown, total_score = _compute_scoring_breakdown(
    ...
    vals["risk_band"] = (
        "low"
        if vals["risk_score"] >= 80
        else ("medium" if vals["risk_score"] >= 60 else "high")
    )

    top = sorted(breakdown, key=lambda x: x["contribution"], reverse=True)[:3]
    vals["top_factors"] = [f"{x['factor']} {x['score']:+.2f}" for x in top]
    vals["capital_relative"] = round(
        _safe_div(vals["equity"], vals["assets"]) or 0.0, 3
    )
...
def credit_decision(analysis_payload: dict, requested_amount: float):
    ...
    recommended = float(
        analysis_payload.get("credit_limit_recommended", 0.0) or 0.0
    )
    ...
    if requested_amount > max_by_sales:
        insights.append(
            "Reduce el monto solicitado por debajo del 35% de las ventas."
        )
    if dscr < 1.2:
        insights.append(
            "Aumenta el flujo de caja operativo o extiende plazo para mejorar la cobertura (DSCR ≥ 1.2)."
        )
    if analysis_payload.get("ig_engagement", 0.0) < 0.01:
        insights.append(
            "Incrementa el engagement (publicaciones regulares y contenido útil)."
        )
    if not insights:
        insights.append(
            "Perfil sólido; mantén disciplina de cobranza y control de gastos."
        )
